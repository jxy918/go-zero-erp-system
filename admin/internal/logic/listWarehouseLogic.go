...
func (l *ListWarehouseLogic) ListWarehouse(req *types.ListWarehouseRequest) (resp *types.ListWarehouseResponse, err error) {
	page := req.Page
	if page <= 0 {
		page = 1
	}
	...
	warehouses, total, err := l.svcCtx.WarehouseModel.List(page, pageSize, req.Name, req.Name)
	if err != nil {
		return nil, err
	}
	...
	for _, warehouse := range warehouses {
		...
		resp.Warehouses = append(resp.Warehouses, item)
	}
